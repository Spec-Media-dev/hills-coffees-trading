...
| 004 | Member Dashboard | `/dashboard` | **RUN A + RUN B + RUN C (2026-09-12) COMPLETE — Phases 1–8 (T001–T025), 25/29 tasks. Only Phase 9 (final closure) remains — NOT YET FORMALLY CLOSED.** RUN A/B established the real module registry, capability-driven shell, live composed overview, and ineligible-state reuse (see prior entries in this doc's history / the handoff doc). RUN C: account-area entries truthfully collapse to the existing `/dashboard/settings` route (no invented Agreements/KYB-status route — neither has a reachable destination for an already-authorized member, documented as a genuine product gap); a dedicated, real-fixture-backed `tests/dashboard/capability-gating.test.ts` (live `organization_can_sell` toggle, no mock, same mechanism Feature 003's own tests use); `tests/dashboard/tenant-isolation.test.ts` proves cross-tenant isolation with REAL, concurrently-fetched multi-org fixture data fed through interleaved `Promise.all` calls (the strongest available proof for these pure, parameter-explicit functions); a real Chrome/CDP pass reconfirmed 0 axe violations, exact landmark counts, keyboard focus, a genuine mobile-drawer focus-trap-and-restore, and a REAL no-JS request (`Emulation.setScriptExecutionDisabled`) showing real nav/main content in the raw HTML; the org-switch-failure feedback question was audited and concluded no code change is needed (the only failure mode is a non-member/tampered id, already safely and silently refused by `setActingOrganization`, and a genuine legitimate race — a membership revoked between render and submit — still leaves the user in an accurate, non-misleading state); the remaining "Account" fallback (`components/public/site-header.tsx`) was confirmed outside Feature 004's surface (public site chrome, never rendered inside `/dashboard/*`) and deliberately left to Feature 002/003. 605/605 tests passing, 53 files; typecheck/build/lint/`git diff --check` all clean. Full evidence: [IMPLEMENTATION-HANDOFF.md](../../specs/004-member-dashboard/IMPLEMENTATION-HANDOFF.md). | [spec](../../specs/004-member-dashboard/spec.md) · [plan](../../specs/004-member-dashboard/plan.md) · [tasks](../../specs/004-member-dashboard/tasks.md) · [handoff](../../specs/004-member-dashboard/IMPLEMENTATION-HANDOFF.md) |
