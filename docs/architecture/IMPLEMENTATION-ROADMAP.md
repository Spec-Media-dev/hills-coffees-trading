...
| 005 | Inventory, Custody & Storage | `/dashboard` + shared layer | **IMPLEMENTED / VERIFIED / CLOSED — 25/25 tasks (T001–T025), 2026-09-21.** DB-OPEN-19 RESOLVED 2026-09-21: migration `20260921120000_feature_005_db_open_19_inventory_variance_hold` applied and live-proven (T014 closed). T022 closed under the accepted exhaustive sequential batched full-suite verification (Feature 012 precedent, 183 canonical test files across 10 non-overlapping sequential batches, 2125 passed / 0 failed / 53 skipped, 0 missing / 0 duplicate / 0 unexpected files, all batches exit 0; clean lint (0 errors, 1 warning), typecheck, build (74/74 static pages), and git diff --check). T023 verified (zero mutations, zero service-role keys in inventory). T024 verified (zero caching, zero public page imports). T025 verified (roadmap updated, DB-OPEN-05 confirmed unchanged/open). Phase 1 (T001–T006), Phase 2 (T007–T012), Phase 3 (T013–T014), Phase 4 (T015), Phase 5 (T016–T019), Phase 6 (T020–T021), and Phase 7 (T022–T025) are complete. Member inventory/storage/history UI live under `/dashboard/inventory`, `/dashboard/storage`, `/dashboard/inventory/history`, reading exclusively through `lib/inventory/*`; registered with Feature 004's module contract — both nav entries AND the bounded "what did I buy"/"where is it" overview cards enter through the SAME `overviewCards` contract. Only two quantity labels exist anywhere in the UI — "Owned quantity"/"Reserved quantity" — no fabricated third "available" figure. DB-OPEN-05 and DB-OPEN-12 both degrade honestly. Full evidence: [tasks.md](../../specs/005-inventory-custody-storage/tasks.md) · [IMPLEMENTATION-HANDOFF.md](../../specs/005-inventory-custody-storage/IMPLEMENTATION-HANDOFF.md).
...
| 006 | Marketplace, Listings & Resale | `/dashboard` | **IMPLEMENTED / VERIFIED / CLOSED — 32/32 tasks (T001–T032), 2026-09-21.** Post-closure T033 (homepage "Recently listed" authorized projection, 2026-09-23) checked. T012 CLOSED under explicit product owner decision (2026-09-21): `SOLD_OUT` listings are excluded from buyer browse/list pages and return `notFound()`/404 on direct buyer URL with no purchase action; seller/admin management views retain access for history/management via `offers_owner_or_admin`; existing implementation in `browse.ts` and `src/app/dashboard/coffee/[offerId]/page.tsx` satisfies this without changes. T015, T018, T023 and T024 are proven LIVE against the real database with a disposable, fully-removed fixture chain (`tests/listings/live-chain.ts`): a real own-organization listing built on a genuinely settled purchase — bought through the real Feature 007 checkout, settled by `admin_review_payment()` (Feature 009's rewrite of 007's function; Feature 008's application layer does not exist and is not claimed), listed through Feature 006's real actions, approved/published by COMPLIANCE. Proven: DRAFT → PENDING_REVIEW with the database-written `listing_status_history` row (and six permitted transitions in order, plus every forbidden edge refused by the trigger, including for a second seller-capable org); reserve 5 kg → actionable 12 → 7, settle → filled 5 / PARTIALLY_FILLED, then fill to SOLD_OUT, with the seller page rendering each state; expiry restores each hold's quantity exactly once (retries, direct and concurrent calls change nothing; no negative reserved; no drift). The earlier "settled-order ceiling" no longer holds. The live blocks are GATED (`F006_LIVE_PROOF=1`; `npx vitest run tests/listings` → 175 pass / 16 skipped, with the flag 23 files / 191 tests) because settlement leaves append-only `inventory_ownership_events`; cleanup is proven by 21-table before/after counts (only ownership events +1/+3 and `audit_logs` grow, by design). Recorded, not acted on: the Feature 010 compliance console offers no PUBLISHED control (fixture used the direct compliance update); `validate_offer_transition`'s state-machine block runs on UPDATE only and has no branch for ARCHIVED/SUSPENDED/SOLD_OUT sources. **No DB/RLS/trigger/grant change and no migration; no service-role in product runtime; no hard deletes.** DB-OPEN-05 and DB-OPEN-12 remain open and unbypassed. Negotiation gap remains in spec.md §Open items. Full history: [tasks.md](../../specs/006-marketplace-listings-resale/tasks.md) · [IMPLEMENTATION-HANDOFF.md](../../specs/006-marketplace-listings-resale/IMPLEMENTATION-HANDOFF.md). Prior: CLOSURE RUN 2026-09-21 (27/32); RUN A 2026-09-12 Phase 1 (T001–T008); RUN B 2026-09-12 Phase 2–4 (T009–T015); RUN C 2026-09-13 Phase 5–7 (T016–T026). | [spec](../../specs/006-marketplace-listings-resale/spec.md) · [plan](../../specs/006-marketplace-listings-resale/plan.md) · [tasks](../../specs/006-marketplace-listings-resale/tasks.md) |
...
| 008 | Payments, Settlement, Invoices & Payouts | `/dashboard` + finance layer | **Payment runtime SUPERSEDED by Feature 013 (bank-transfer commerce, 2026-09-24); the 10 open 008 tasks are NOT completed and are not to be implemented. Historical 008 migrations/rollback/postflight/specs remain byte-identical (Feature 013 T015 guard).** Prior status: **29/39 tasks; not closed (2026-09-22 DB gate closure).** Phase 1 (T001–T006), T008, T009 (human-reviewed & approved DB design), T011 (migration 20260922120000 applied to linked DB, postflight 20/20 PASS), T014–T016, T018–T020, T022, T023, T025–T028, T031–T036, T038, T039 complete. Remaining 10 tasks gated on live Stripe credentials/Connect setup (T007, T010, T012, T013, T017, T021, T029, T030, T037) or manual-fallback business decision (T024). | [spec](../../specs/008-payments-settlement-invoices-payouts/spec.md) · [plan](../../specs/008-payments-settlement-invoices-payouts/plan.md) · [tasks](../../specs/008-payments-settlement-invoices-payouts/tasks.md) · [handoff](../../specs/008-payments-settlement-invoices-payouts/IMPLEMENTATION-HANDOFF.md) |
...
| 010 | Operations / Admin Console | `/dashboard-admin` | **Finance areas payments/payouts/invoices (blocked on feature-008-finance-layer) → delivered by Feature 013 Phase 4.** **IN PROGRESS — 50 / 59 tasks per `tasks.md` (Final non-payment closure 2026-09-23: T059 fixtures + test taxonomy CLOSED; T056 tag translations awaits the reviewed migration apply; T013–T015/T031–T033 Feature 008-dependent; T038/T041 open). Earlier row text: 47 / 55 tasks (Catalogue Media & Bilingual Content Live Verification 2026-09-23: T053 and T054 VERIFIED and CLOSED — migration applied, 12/12 postflight checks ok, 27/27 live verification checks passed end-to-end; Provider-Independent Final Verification 2026-09-23: T039 and T040 VERIFIED and CLOSED; RUN F010-ACCOUNT-MEDIA 2026-09-22: T047, T048, T050, T051, T052, T055 closed; Price-administration run 2026-09-21: T049 closed). NOT closed.** T012 COMPLETE 2026-09-19: the Compliance dispute review surface composes Feature 012's closed dispute layer (`transition_dispute()` + `dispute_status_history`), live + Chrome/axe proven; T032's dispute half proven, T032 still blocked on T014 (Feature 008). Built and live-verified: RUN A shell/guards/overview/account (T001–T006, T046), RUN B compliance KYB/listing decisions (T007–T009, T011), RUN D warehouse oversight over Feature 009's named operations (T016–T020), RUN E catalogue + auditor read-only (T021–T026), RUN F system configuration (T028, T042–T045), RUN G automated suites (T030, T034, T035), RUN H state coverage + console-wide a11y/RTL/responsive (T036, T037), RUN J organizations compliance read (T010), Database hygiene M1 config attribution (T027, T029), RUN F010-ACCOUNT-MEDIA logo/avatar/listing media/email (T047, T048, T050, T051), Final Verification (T039, T040), and Catalogue Media & Translations (T053, T054). **Open, by cause**: Feature 008 finance layer absent → T013–T015, T031, T032 (payment half) BLOCKED (payments/payouts/invoices areas render the honest `blocked` placeholder); DB-OPEN-09 (no approved dispute freeze mechanism) → stated in the dispute console as record-label-only, never simulated; DB-OPEN-06 → auditor audit-log read is a stated capability gap; DB-OPEN-22 organizations half resolved (T010 complete); its `file_assets` half keeps KYB evidence bytes unlocatable for COMPLIANCE; DB-OPEN-21 resolved (config changes audited) → T027/T029 COMPLETE; T033 PARTIAL (Phases 3–9 not all closed); Phase 12 T038 (full suite exit-0) and T041 (roadmap reconciliation) open pending overall feature completion. OPS-01 (no dual control), SHIP-OPEN-01 (`shipping_rules` unconsumed), COMMISSION-OPEN-01 (0% fallback undecided), DB-OPEN-19 (no variance/reconciliation model), SUSPEND-OPEN-01 (suspended-organization mid-operation policy), DB-BLOCK-01 (bytes outside KYB scope) all remain OPEN and are stated in-product where the operator would otherwise assume the capability exists. DB-BLOCK-07 remains RESOLVED by Feature 009 (unchanged). **No service role in product runtime; no hard deletes.** | [spec](../../specs/010-admin-operations-console/spec.md) · [plan](../../specs/010-admin-operations-console/plan.md) · [tasks](../../specs/010-admin-operations-console/tasks.md) |
...
| DB-OPEN-19 | No variance / reconciliation / quarantine model for warehouse custody (Feature 010 RUN D, 2026-09-16). **RESOLVED 2026-09-21 (Feature 005 T014): migration `20260921120000_feature_005_db_open_19_inventory_variance_hold` applied and live-proven.** Feature 010 warehouse screens for recording/resolving cases remain to be built. | 010 (warehouse reconciliation), 012 | Reconciliation workflow |
