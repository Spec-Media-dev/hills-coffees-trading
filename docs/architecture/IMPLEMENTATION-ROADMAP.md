...
| 004 | Member Dashboard | `/dashboard` | **RUN A (2026-09-12) COMPLETE — Phase 1 Module Contract (T001–T003) + Phase 2 Shell Layout (T004–T008), 8/29 tasks.** `lib/dashboard/modules.ts`/`registry.tsx`/`overview.tsx` establish the module-registration contract 005–009/012 will plug into (declaration is presentational only, never authorization — SEC-002); `dashboard/layout.tsx`'s previously-hardcoded `canSell: false` stub is replaced with real, per-request `organization.canBuy`/`canSell` resolution via `components/dashboard/sidebar.tsx`; `components/dashboard/topbar.tsx` adds an account menu and a reserved (genuinely inert) notifications entry; `components/dashboard/responsive/table-card-list.tsx` gives future modules a shared table/card-list primitive. Phases 3–9 (T009–T029 — acting-org switcher, the real overview page, ineligible-member states, account-area entries, formal test/closure phases) remain NOT STARTED. Full evidence: [IMPLEMENTATION-HANDOFF.md](../../specs/004-member-dashboard/IMPLEMENTATION-HANDOFF.md). | [spec](../../specs/004-member-dashboard/spec.md) · [plan](../../specs/004-member-dashboard/plan.md) · [tasks](../../specs/004-member-dashboard/tasks.md) · [handoff](../../specs/004-member-dashboard/IMPLEMENTATION-HANDOFF.md) |
