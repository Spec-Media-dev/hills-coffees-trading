...
| 012 | Disputes, Notifications & Audit | `/dashboard` + `/dashboard-admin` | **IN PROGRESS — 23 / 28 tasks (RUN D readiness, 2026-09-19). NOT closed.** Built and live-verified: member dispute raise/track + text evidence (T001–T003, T005–T008), compliance review domain (T004 — **PARTIAL**, DB-OPEN-23), honest notification read surface + own-user preferences + topbar entry (T009–T012), shared read-only history/audit layer integrated into 005/006/007 (T013–T016), nav registration (T017), isolation / role / immutability / honest-limitation / escaping / state-coverage tests (T018–T023), Feature-012-wide a11y/RTL pass (T024, 108 surfaces). **T025–T028 verification is GREEN but cannot close: `Depends: all` is unmet while T004 is PARTIAL.** Open: DB-BLOCK-01 (dispute evidence bytes), DB-BLOCK-04, DB-OPEN-06, DB-OPEN-09 (**MKT-07 not fully satisfiable**), DB-OPEN-23. | [spec](../../specs/012-disputes-notifications-audit/spec.md) · [plan](../../specs/012-disputes-notifications-audit/plan.md) · [tasks](../../specs/012-disputes-notifications-audit/tasks.md) |
...
| DB-OPEN-21 | Configuration UPDATEs (`platform_admins`, commission, tax, shipping, `payment_accounts`) persist no actor — no `updated_by`, no audit trigger (Feature 010 RUN F, 2026-09-17; decision D2 ii — not resolved) | 010 T027/T029 (PARTIAL) | Attributable privileged changes |
| DB-OPEN-22 | A pure COMPLIANCE role cannot read `organizations` / `file_assets` / `account_status_history` — suspension UPDATE affects zero rows; KYB bytes unlocatable for that role (Feature 010 RUN B; ID assigned RUN H) | 010 T010 | Compliance suspension + evidence review by the COMPLIANCE role |
| DB-OPEN-23 | Dispute status transitions are neither attributable (only resolve/reject persist actor+reason) nor DB-guarded (no trigger on `disputes`; `disputes_ops_update` allows any column) — Feature 012 RUN A, 2026-09-19; human decision required | 012 T004 (PARTIAL) → T025–T028 | MKT-07 attributable resolution; Feature 012 closure |
