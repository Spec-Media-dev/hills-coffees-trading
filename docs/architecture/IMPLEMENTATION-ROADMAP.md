...
| 011 | Pricing & Reference Data | Public + member + admin | **IMPLEMENTED — 19 / 22 tasks (RUN A 2026-09-20; T013 closed 2026-09-21); NOT closed.** The four branded price types, the licence-gated cached read layer (`lib/pricing/*`, tag `reference-prices`), the disclosure-complete presentation contract and components, and the homepage integration (Feature 002's PRICE-011 placeholder is now fed by the contract) are built; raw values only — no conversion (DB-OPEN-08); real Chrome + axe across EN/AR × light/dark × 390/1366 clean. **DB-BLOCK-10 remainder RESOLVED 2026-09-20**: migration `20260920160000_feature_011_db_block_10_price_policy_scope.sql` (applied; postflight 9/9; live 20/20) narrowed three `price_*_admin` policies to `authenticated`, so anonymous reads of the pricing tables work; the seeded live homepage (approved benchmark + stale feed + explanatory basis, exact stored values) is proven in real Chrome + axe. T013 COMPLETE 2026-09-21: Feature 010 T049's price administration calls `revalidateReferencePrices()` after every successful change (running-server proof: a direct DB write stays cached; a console change shows on the next request). Open: T020 (`npm test` cannot exit 0 on the current tree — two design/public test failures from the homepage-redesign work, outside 010/011) and T021–T022 (`Depends: T020`). New gap recorded: QUOTE-OPEN-01 (no Hills quote entity). | [spec](../../specs/011-pricing-reference-data/spec.md) · [plan](../../specs/011-pricing-reference-data/plan.md) · [tasks](../../specs/011-pricing-reference-data/tasks.md) |
