...
| 005 | Inventory, Custody & Storage | `/dashboard` + shared layer | **RUN B, RECONCILED (2026-09-12) — Phase 1 (T001–T006, RECONCILED) + Phase 2 (T007–T012) + Phase 4 (T015), 13/25 tasks. NOT complete, NOT closed (Phase 3/5/6/7 remain).** Member inventory/storage/history UI now live under `/dashboard/inventory`, `/dashboard/storage`, `/dashboard/inventory/history`, reading exclusively through `lib/inventory/*`; registered with Feature 004's module contract — both nav entries AND the bounded "what did I buy"/"where is it" overview cards now enter through the SAME `overviewCards` contract (extended to allow an async module, the smallest safe change; no page-specific second integration path). The storage page also resolves and shows its originating order's code (a genuine, empirically-proven member-readable `storage_allocations → order_items → orders` chain, unlike DB-OPEN-12) as plain reference text — no order-detail route exists yet to link to. Only two quantity labels exist anywhere in the UI — "Owned quantity"/"Reserved quantity" — no fabricated third "available" figure. DB-OPEN-05 and DB-OPEN-12 both degrade honestly with no crash and no fabricated value. 650/650 tests passing, clean build/typecheck/lint. No real browser/CDP pass yet (every inventory table remains genuinely empty pre-Features 007/008) — reported honestly, not claimed. Full evidence: [IMPLEMENTATION-HANDOFF.md](../../specs/005-inventory-custody-storage/IMPLEMENTATION-HANDOFF.md)'s RUN B + reconciliation sections.
