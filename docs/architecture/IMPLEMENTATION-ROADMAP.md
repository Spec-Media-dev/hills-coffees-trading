...
| 008 | Payments, Settlement, Invoices & Payouts | `/dashboard` + finance layer | **12/39 tasks; not closed (2026-09-22 closure run).** Phase 1 (T001–T006) + T022 were already complete; this run closed T023 (documents/payout presentation — proven against a REAL settled MEMBER_SELLER sale, not fabricated), T025 (dashboard registry — `payments`/`payouts` nav), T026 (EN/AR copy for what's implemented), T027 (read isolation/role-negative tests) and T033 (state coverage). `/dashboard/payouts` is new. **Provider selection (Phase 2, T007–T010) remains a genuine external Finance/Legal/Banking decision** — engineering cannot resolve it; Phases 3–4 (T011–T021, the trusted-funding/settlement gate) stay blocked on that decision and cannot be worked around. T024 (manual-proof fallback) stays conditional/open pending an explicit business + Storage-design decision. T028 (snapshot immutability across a commission-policy mutation), T032 (single-settlement-caller audit, needs T018), T034 (real browser/axe), T035 (production-build exposure proof), T029–T031/T037 (provider/settlement tests) remain open, each for a stated, specific reason — none converted into fake completion. | [spec](../../specs/008-payments-settlement-invoices-payouts/spec.md) · [plan](../../specs/008-payments-settlement-invoices-payouts/plan.md) · [tasks](../../specs/008-payments-settlement-invoices-payouts/tasks.md) · [handoff](../../specs/008-payments-settlement-invoices-payouts/IMPLEMENTATION-HANDOFF.md) |
