...
/**
 * Feature 010 T004 — the `(compliance)` route group's OWN server-side guard. Every route under this
 * group is refused here, on every request, unless `is_compliance_operator()` is true for the operator's own
 ...
 */
export default async function ComplianceAreaLayout({ children }: { children: React.ReactNode }) {
  const access = await checkGroupAccess("compliance");
  if (!access.ok) {
    return <AdminAccessDenied denial={access.denial} requiredFunction={ADMIN_GROUP_ROLE_FUNCTIONS.compliance} />;
  }
  return children;
}
