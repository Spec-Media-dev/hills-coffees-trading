...
/**
 * Feature 010 T004 — the `(system)` route group's OWN server-side guard. Every route under this
 ...
 */
export default async function SystemAreaLayout({ children }: { children: React.ReactNode }) {
  const access = await checkGroupAccess("system");
  if (!access.ok) {
    return <AdminAccessDenied denial={access.denial} requiredFunction={ADMIN_GROUP_ROLE_FUNCTIONS.system} />;
  }
  return children;
}
