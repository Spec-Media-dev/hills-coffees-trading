...
/**
 * Feature 010 T004 — the `(audit)` route group's OWN server-side guard. Every route under this
 * group is refused here, on every request, unless `is_auditor()` is true for the operator's own
 ...
 */
export default async function AuditAreaLayout({ children }: { children: React.ReactNode }) {
  const access = await checkGroupAccess("audit");
  if (!access.ok) {
    return <AdminAccessDenied denial={access.denial} requiredFunction={ADMIN_GROUP_ROLE_FUNCTIONS.audit} />;
  }
  return children;
}
