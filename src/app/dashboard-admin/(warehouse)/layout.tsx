...
/**
 * Feature 010 T004 — the `(warehouse)` route group's OWN server-side guard. Every route under this
 * group is refused here, on every request, unless `is_warehouse_operator()` is true for the operator's own
 ...
 */
export default async function WarehouseAreaLayout({ children }: { children: React.ReactNode }) {
  const access = await checkGroupAccess("warehouse");
  if (!access.ok) {
    return <AdminAccessDenied denial={access.denial} requiredFunction={ADMIN_GROUP_ROLE_FUNCTIONS.warehouse} />;
  }
  return children;
}
