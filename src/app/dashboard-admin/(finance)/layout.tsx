...
/**
 * Feature 010 T004 — the `(finance)` route group's OWN server-side guard. Every route under this
 * group is refused here, on every request, unless `is_finance_operator()` is true for the operator's own
 ...
 */
export default async function FinanceAreaLayout({ children }: { children: React.ReactNode }) {
  const access = await checkGroupAccess("finance");
  if (!access.ok) {
    return <AdminAccessDenied denial={access.denial} requiredFunction={ADMIN_GROUP_ROLE_FUNCTIONS.finance} />;
  }
  return children;
}
