...
describe("T019 — the rules actually bite (mutation checks on synthetic SQL)", () => {
  ...
  it.each([
    ...
    ["authenticated column-level write grant", good.replace("grant select on table public.t", "grant update (id) on table public.t")],
    ["authenticated column-level select plus write", good.replace("grant select on table public.t", "grant select (id), insert (id) on table public.t")],
