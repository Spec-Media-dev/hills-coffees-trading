/**
 * The Arabic sibling of the public copy dictionary (Phase 5.5, UIF-017 — plan §8; `CONTENT-AR-01`).
 *
 * SERVER-SAFE BY CONSTRUCTION, exactly like `en.ts`: no client directive, no `react` import, no
 * `i18next` import. This is a *sibling of the same module*, not a second dictionary system — the
 * shape is derived from `en` and checked by the compiler, so a key that does not exist in English
 * cannot be invented here.
 *
 * ────────────────────────────────────────────────────────────────────────────────────────────────
 * SCOPE — amended by the public design convergence pass (2026-09-10)
 * ────────────────────────────────────────────────────────────────────────────────────────────────
 *
 * UIF-017 shipped the EN/العربية infrastructure with interface chrome only, because approved Arabic
 * *content* had not been supplied (`CONTENT-AR-01`). The product owner has since directed that the
 * public experience must not show English business copy to an Arabic visitor, and authorised this
 * pass to carry the approved English across.
 *
 * What that authorisation does and does not mean:
 *
 *   ✔ Every string here is a FAITHFUL RENDERING of the reviewed English value of the same key.
 *     Meaning, scope and hedging are preserved sentence by sentence. Where the English says "only
 *     what we can evidence", the Arabic says the same and no more.
 *
 *   ✘ No Arabic claim exists that has no English counterpart. No figure, certification, partner,
 *     origin count, legal statement or capability was added. Business names, routes and the Trading
 *     Portal name are transliterated consistently.
 *
 *   ⚠ `CONTENT-AR-01` is NOT closed by this file. It is re-scoped: the translations are supplied and
 *     the remaining obligation is Content/Legal sign-off of the wording before production. Until
 *     then the source of truth for meaning remains `en.ts`, and any conflict is resolved toward the
 *     English.
 *
 * Register: professional Modern Standard Arabic for a B2B audience — no colloquialism, no marketing
 * hyperbole beyond what the English carries. Hills Coffee is written as "هيلز كوفي".
 */
import type { DeepPartial, PublicCopy } from "./types";

export const ar: DeepPartial<PublicCopy> = {
  site: {
    name: "هيلز كوفي",
    tagline: "توريد البن الأخضر من دبي إلى المنطقة العربية.",
  },

  a11y: {
    skipToContent: "تخطَّ إلى المحتوى الرئيسي",
    primaryNavigation: "التنقل الرئيسي",
    footerNavigation: "روابط التذييل",
    homeLink: "هيلز كوفي — الصفحة الرئيسية",
  },

  nav: {
    coffee: "البن",
    origins: "المناشئ",
    sourcing: "التوريد",
    about: "من نحن",
    contact: "تواصل معنا",
    portalEntry: "بوابة التداول",
    marketplace: "السوق",
  },

  megaMenu: {
    coffee: {
      title: "كتالوج القهوة",
      body: "معلومات عن كل قهوة تنشرها هيلز — المنشأ والمعالجة والتعبئة وسياق الجودة. ليست قائمة بعروض مباشرة.",
      primary: "تصفح الدليل",
    },
    marketplace: {
      title: "سوق الأعضاء",
      body: "إدراجات مباشرة من بائعين خضعوا للمراجعة — السعر لكل كيلوغرام والكمية والتوفر — للأعضاء المعتمدين في بوابة تداول هيلز.",
      primary: "افتح السوق",
      primaryGuest: "شاهد أحدث الإدراجات",
      badge: "للأعضاء",
      createAccount: "أنشئ حسابًا",
      signIn: "تسجيل الدخول",
      catalogueHint: "تبحث عن معلومات القهوة؟",
    },
    origins: {
      title: "مناطق الإنتاج",
      body: "المناطق التي تورّد منها هيلز، وكل منها مرتبط بالبن المنشور منها.",
      primary: "استعرض المناشئ",
    },
    sourcing: {
      title: "كيف نورّد البن ونحتفظ به",
      body: "علاقات المنشأ، والحيازة، والخدمات اللوجستية، وتوثيق الجودة، بعبارات واضحة.",
      primary: "اقرأ كيف نعمل",
    },
    about: {
      title: "هيلز كوفي",
      body: "شركة لتوريد البن الأخضر وتداوله، وُلدت في دبي وتخدم المنطقة العربية.",
      primary: "عن هيلز",
    },
    relatedHeading: "انظر أيضًا",
  },

  cta: {
    requestAnOffer: "اطلب عرض سعر",
    exploreAllCoffee: "استكشف كل البن",
  },

  gallery: {
    region: "معرض الصور",
    previous: "الصورة السابقة",
    next: "الصورة التالية",
    show: "عرض الصورة {n} من {total}",
    position: "الصورة {n} من {total}",
  },
  controls: {
    switchToDark: "التبديل إلى المظهر الداكن",
    switchToLight: "التبديل إلى المظهر الفاتح",
    languageSwitcher: "Switch to English",
    languageSwitcherShort: "EN",
    openMenu: "فتح القائمة",
    closeMenu: "إغلاق القائمة",
    menuTitle: "القائمة",
    openSearch: "بحث",
    searchFieldLabel: "ابحث عن بن أو منشأ",
    searchTitle: "ابحث في بن هيلز",
    searchHint: "سيفتح بحثك دليل البن المنشور.",
    searchPlaceholder: "ابحث حسب البن أو المنشأ",
    searchSubmit: "تصفح دليل البن",
    close: "إغلاق",
  },

  footer: {
    rights: "جميع الحقوق محفوظة.",
    exploreHeading: "استكشف",
    companyHeading: "الشركة",
    accountHeading: "الحساب",
    commercialHeading: "تعامل مع هيلز",
    closingLine: "بن أخضر، يُورَّد من المنشأ ويُحفظ في دبي.",
    brandStatement:
      "بن أخضر قابل للتتبع لمحامص البن والمستوردين والموزعين في أنحاء المنطقة العربية.",
    commercialBody:
      "شاركنا نوع البن والكمية والتوقيت ونقطة التسليم التي تعمل عليها، وسيتابع فريقنا التجاري الحديث معك مباشرة.",
    locationLine: "دبي · مصر",
  },

  media: {
    placeholderLabel: "الصور قادمة قريبًا",
    placeholderDescription: "لم تُنشر صور هذا العنصر بعد.",
  },

  home: {
    hero: {
      eyebrow: "توريد البن الأخضر وتداوله",
      headline: "بن أخضر، يُورَّد من المنشأ ويُحفظ في دبي.",
      lead:
        "تورّد هيلز كوفي البن الأخضر من مناطق الإنتاج وتحتفظ به في حيازة معتمدة من هيلز. تتعاقد معنا محامص البن والمستوردون والموزعون في أنحاء المنطقة العربية للحصول على إمداد يمكنهم تتبعه إلى مكان زراعته.",
      exploreAction: "استكشف البن",
      imageAlt: "مزارع بن يعمل بين أحواض التجفيف المرتفعة في المنشأ.",
      landscapeAlt: "مزارع يقطف ثمار البن الناضجة على سفح تل يطل على وادٍ جبلي.",
      scrollCue: "مرّر للأسفل",
    },
    intents: {
      eyebrow: "ابدأ من هنا",
      title: "ثلاث طرق للعمل معنا",
      lead: "كل مسار يقودك إلى وجهة مختلفة. اختر ما يناسب احتياجك اليوم.",
      source: {
        title: "توريد البن",
        body:
          "أخبرنا بالمواصفات والكمية ونقطة التسليم التي تحتاجها. نتوجه إلى مناطق الإنتاج ونعود إليك بما يمكننا توريده.",
        action: "أرسل طلب توريد",
      },
      explore: {
        title: "استكشف البن المتاح",
        body: "تصفح أنواع البن التي تنشرها هيلز، كلٌّ منها مع منشئه وطريقة معالجته وتعبئته وشهاداته.",
        action: "اطّلع على البن",
      },
      trade: {
        title: "تعامل مع هيلز",
        body:
          "يشتري الأعضاء المعتمدون البن ويحتفظون به ويعيدون بيعه عبر بوابة هيلز للتداول. تُراجَع العضوية قبل تفعيل أي تداول.",
        action: "عن العضوية",
      },
    },
    credibility: {
      eyebrow: "كيف نعمل",
      title: "إمداد يمكنك الاعتماد عليه",
      lead:
        "البن الأخضر التزام برأس المال قبل أشهر من وصوله إلى المحمصة. هذه هي أجزاء ذلك الالتزام التي نتحمل مسؤوليتها.",
      origin: {
        title: "علاقات مباشرة مع المنشأ",
        body: "نشتري من مناطق إنتاج نعرفها، ونحافظ على الصلة بين البن ومكان زراعته.",
      },
      quality: {
        title: "جودة موثقة",
        body: "تنتقل الشهادات وأدلة الجودة مع البن. لا ننشر إلا ما يمكننا إثباته وما نحن مخولون بالإفصاح عنه.",
      },
      custody: {
        title: "حيازة حتى نقل الملكية",
        body:
          "يبقى البن في حيازة معتمدة من هيلز حتى تنتقل الملكية إلى المشتري، فلا تكون المسؤولية غامضة أبدًا.",
      },
      membership: {
        title: "عضوية مُراجَعة",
        body: "يُراجَع كل حساب تداول ويُعتمد قبل أن يتمكن من إجراء أي معاملة. التسجيل وحده لا يخوّل التداول أبدًا.",
      },
      imageAlt: "ثمار بن طازجة مجموعة في المنشأ بعد الحصاد.",
    },
    featuredCoffee: {
      eyebrow: "البن",
      title: "ما نحمله حاليًا",
      lead: "مجموعة مختارة من أنواع البن التي تنشرها هيلز حاليًا.",
      action: "اطّلع على كل البن",
      empty: "لا يوجد بن منشور في الوقت الحالي.",
    },
    featuredOrigins: {
      eyebrow: "المناشئ",
      title: "من أين يأتي",
      lead: "مناطق الإنتاج التي يأتي منها البن الذي نحمله.",
      action: "اطّلع على كل المناشئ",
      empty: "لا توجد مناشئ منشورة في الوقت الحالي.",
    },
    story: {
      eyebrow: "من الثمرة إلى البن الأخضر",
      title: "ما يحدث قبل نشر أي دفعة",
      lead: "أربع لحظات في الطريق من الشجرة إلى بن يمكن لهيلز نشره. في كل واحدة منها يُصنع جزء من سجل البن.",
      listLabel: "مراحل البن",
      previous: "المرحلة السابقة",
      next: "المرحلة التالية",
      positionLabel: "المرحلة {current} من {total}",
      items: {
        cherry: {
          title: "ثمار تُقطف في المنشأ",
          body:
            "تُنتقى الثمار الناضجة يدويًا في مناطق الإنتاج التي تشتري منها هيلز. من هنا تبدأ الصلة بين البن ومكان زراعته.",
          alt: "مزارع ينتقي ثمار البن الحمراء الناضجة يدويًا من الغصن.",
        },
        drying: {
          title: "التجفيف والمعالجة",
          body: "تُعالَج الثمار وتُجفَّف في المنشأ. وتنتقل طريقة المعالجة مع البن إلى ملفه المنشور.",
          alt: "بن يجف على أحواض طويلة مرتفعة تحت منطقة معالجة مغطاة في المنشأ.",
        },
        inspection: {
          title: "الفرز والفحص",
          body: "يُفحص البن الأخضر قبل نشره. تُراجَع الشهادات وأدلة الجودة، وتُشارَك مع المشترين المعتمدين.",
          alt: "يد تفحص حبوب البن الأخضر داخل كيس خيش مفتوح.",
        },
        green: {
          title: "بن أخضر جاهز للتوريد",
          body: "ينتقل البن الأخضر المُجهَّز إلى حيازة معتمدة من هيلز ويبقى فيها حتى تنتقل الملكية إلى المشتري.",
          alt: "حبوب بن أخضر تنسكب من كيس خيش مفتوح على سطح خشبي.",
        },
      },
    },
    showcase: {
      eyebrow: "البن",
      title: "ما يحمله كل بن منشور",
      lead: "لا تنشر هيلز أي بن إلا ومعه سجله. الدليل مرجع للتوريد، وليس دفتر طلبات مباشرًا.",
      origin: {
        title: "المنشأ والمنطقة",
        body: "مكان زراعة البن، مرتبطًا بملف منشأ منشور.",
      },
      process: {
        title: "طريقة المعالجة والصنف",
        body: "كيف تحولت الثمرة إلى بن أخضر، وما الذي زُرع.",
      },
      packaging: {
        title: "التعبئة والشهادات",
        body: "كيف يُشحن، والوثائق التي تنتقل معه.",
      },
      action: "اطّلع على كل البن",
      alt: {
        origin: "سلة حصاد مليئة بثمار البن وخلفها تلال يغطيها الضباب.",
        process: "بن أخضر وآخر مجفف مرصوصان في صفوف على منضدة خشبية وخلفهما الحقول.",
        packaging: "كيس خيش وبن أخضر وثمار طازجة على حافة حجرية في ضوء دافئ.",
      },
    },
    marquee: {
      eyebrow: "في المنشأ",
      title: "البن كما نراه قبل أن يصبح دفعة",
      trackLabel: "صور من المنشأ",
      captions: {
        cherries: "ثمار على الغصن",
        basket: "سلة الحصاد",
        ripening: "ثمار في طور النضج",
        sorting: "فرز البن الأخضر",
        sackCherries: "بن أخضر وثمار",
        beds: "أحواض تجفيف مرتفعة",
        grower: "مزارع في المنشأ",
        hands: "قطف يدوي",
      },
    },
    traceability: {
      eyebrow: "التتبع",
      title: "الصلة بمكان الزراعة",
      lead: "تحافظ هيلز على الصلة بين البن ومنشئه دون انقطاع، من منطقة الإنتاج إلى المشتري الذي تنتقل إليه الملكية.",
      imageAlt: "بن يجف على أحواض مرتفعة في ضوء المساء الدافئ في المنشأ.",
      chainLabel: "سلسلة المسؤولية",
      links: {
        origin: "أين زُرع",
        quality: "ما ينتقل معه",
        custody: "من يحوزه",
        membership: "من يمكنه تداوله",
      },
    },
    journey: {
      eyebrow: "كيف تعمل هيلز",
      title: "من منطقة الإنتاج إلى نقطة التسليم",
      lead: "أربع مراحل، كل واحدة منها محسوبة. لا شيء هنا يصف قدرة لا تمارسها هيلز.",
      stepLabel: "المرحلة",
      alt: {
        relationships: "مزارع يقف بين شجيرات البن وخلفه الجبال.",
        custody: "أكياس خيش من البن الأخضر مرصوصة على منصات تحت سقف عالٍ.",
        logistics: "أكياس بن أخضر مصفوفة تحت نوافذ مقوسة عالية.",
        quality: "بن أخضر منثور على طاولة فرز للفحص.",
      },
    },
    marketplace: {
      eyebrow: "سوق الأعضاء",
      title: "حيث يتداول الأعضاء المعتمدون قهوة حقيقية",
      lead: "يعرض سوق بوابة تداول هيلز دفعات فعلية محفوظة تحت عهدة معتمدة من هيلز، يقدّمها بائعون خضعوا للمراجعة إلى مشترين معتمدين.",
      catalogueTitle: "كتالوج القهوة",
      catalogueBody: "معلومات عن القهوة التي تورّدها هيلز — المنشأ والمعالجة وسياق الجودة. متاح للجميع؛ وليس قائمة بعروض مباشرة.",
      catalogueAction: "تصفّح الكتالوج",
      marketplaceTitle: "السوق",
      marketplaceBody: "إدراجات بائعين حقيقية بالسعر لكل كيلوغرام والكمية المتاحة والحالة — تظهر للأعضاء المعتمدين فقط.",
      recentTitle: "أُدرجت حديثًا",
      recentLead: "أحدث خمسة إدراجات في السوق.",
      lockedTitle: "الإدراجات متاحة للأعضاء المعتمدين",
      lockedBody: "تبقى الأسعار والكميات وبيانات البائعين داخل بوابة التداول. أنشئ حسابًا، وأعدّ منظمتك، وأكمل مراجعة هيلز لرؤية الإدراجات المباشرة.",
      createAccount: "أنشئ حسابًا",
      signIn: "تسجيل الدخول",
      howItWorks: "كيف تعمل العضوية",
      pendingTitle: "منظمتك غير معتمدة للتداول بعد",
      pendingBody: "تظهر الإدراجات المباشرة هنا بعد اكتمال مراجعة منظمتك. تابع من حسابك.",
      pendingAction: "اذهب إلى حسابك",
      mfaTitle: "أكمل تسجيل الدخول",
      mfaBody: "أكمل التحقق الثنائي لرؤية الإدراجات المباشرة.",
      mfaAction: "تحقّق الآن",
      operatorTitle: "أنت مسجّل الدخول بصفة مشغّل",
      operatorBody: "تُراجَع الإدراجات وتُدار من وحدة التشغيل.",
      operatorAction: "افتح وحدة التشغيل",
      emptyTitle: "لا توجد إدراجات مباشرة حاليًا",
      emptyBody: "تظهر الإدراجات الجديدة هنا فور نشرها من البائعين.",
      viewAll: "عرض كل الإدراجات",
    },
    reference: {
      eyebrow: "معلومات مرجعية",
      title: "أسعار مرجعية، منفصلة تمامًا",
      lead: "تُنشر المؤشرات المرجعية بمعزل عن عروض هيلز التجارية وعن أسعار إعادة البيع بين الأعضاء. ولا تكون الرقم نفسه أبدًا.",
      conceptsHeading: "أربعة أسعار لا تختلط أبدًا",
      concepts: {
        benchmark: { title: "المؤشر المرجعي", body: "مؤشر سوق خارجي يُعرض مع مصدره وتأخّره ووقت رصده. للمعلومات فقط." },
        quote: { title: "عرض هيلز التجاري", body: "ما تعرضه عليك هيلز لقهوة وكمية وتسليم محددين — عبر محادثة تجارية." },
        listing: { title: "سعر إدراج العضو", body: "سعر يطلبه بائع على دفعة محددة، يظهر فقط للأعضاء المعتمدين في بوابة التداول." },
        executed: { title: "سعر الصفقة المنفّذة", body: "ما اتُّفق عليه فعلًا، ويُسجَّل على الطلب كحقيقة تاريخية." },
      },
      stageLabel: "المؤشر المرجعي",
      stageState: "لم يُنشر بعد",
      stageNote: "عندما تنشر هيلز مؤشرًا مرجعيًا سيظهر هنا مع مصدره وتاريخ رصده، وسيبقى مع ذلك ليس عرضًا.",
    },
    rfq: {
      title: "أخبرنا بما تحتاجه",
      lead: "أرسل نوع البن والكمية والتوقيت ونقطة التسليم التي تعمل عليها، وسيتابع أحد أعضاء الفريق التجاري الأمر من هناك.",
      stepsHeading: "ما الذي يحدث بعد ذلك",
      steps: {
        send: { title: "أرسل طلبك", body: "القهوة أو المنشأ والكمية والتوقيت ونقطة التسليم التي تعمل عليها." },
        conversation: { title: "محادثة توريد", body: "يعود إليك الفريق التجاري بما تستطيع هيلز توفيره وبأي شروط." },
        allocation: { title: "التخصيص والتسليم", body: "تُخصَّص القهوة المتفق عليها وتُسلَّم تحت عهدة معتمدة من هيلز." },
      },
      imageAlt: "ثمار بن ناضجة على غصن بجانب بن أخضر فوق كيس خيش.",
    },
  },

  about: {
    eyebrow: "عن هيلز",
    title: "شركة بن أخضر أُسست للمنطقة العربية",
    lead:
      "تورّد هيلز كوفي البن الأخضر من مناطق الإنتاج وتحتفظ به في حيازة معتمدة من هيلز في دبي، لمحامص البن والمستوردين والموزعين الذين يريدون إمدادًا يمكنهم الاعتماد عليه.",
    identity: {
      title: "وُلدت في دبي، وتركز على المنشأ",
      body: "هيلز شركة وُلدت في دبي ولها مكتب تشغيلي في مصر. نشتري من مناطق إنتاج نعرفها ونحافظ على الصلة بين البن ومكان زراعته.",
    },
    model: {
      title: "شريك توريد، لا بورصة",
      body:
        "الموقع العام هو حيث يبدأ حديث التوريد. أما التداول فيتم داخل بوابة هيلز للتداول بين أعضاء رُوجعت حساباتهم واعتُمدت.",
    },
    custody: {
      title: "حيازة يمكنك الإشارة إليها",
      body: "يبقى البن الذي تورّده هيلز في حيازة معتمدة من هيلز حتى تنتقل الملكية إلى المشتري، فلا تُقسَّم المسؤولية عنه ولا تبقى غامضة.",
    },
    evidence: {
      title: "فقط ما يمكننا إثباته",
      body: "لا تُنشر الشهادات وأدلة الجودة ومعلومات المنشأ إلا عندما تستطيع هيلز إثباتها وتكون مخولة بالإفصاح عنها.",
    },
    glance: {
      heading: "هيلز في لمحة",
      originLabel: "منشأ العلامة",
      originValue: "دبي",
      officeLabel: "المكتب التشغيلي",
      officeValue: "مصر",
      buyersLabel: "لمن نورّد",
      buyersValue: "المحامص والمستوردون والموزعون في أنحاء المنطقة العربية",
      tradeLabel: "كيف يتم التداول",
      tradeValue: "محادثات تجارية، وبوابة تداول للأعضاء المعتمدين بعد المراجعة",
    },
    principlesEyebrow: "كيف تعمل هيلز",
    principlesTitle: "أربعة التزامات وراء كل قهوة ننشرها",
    portalAction: "عن بوابة التداول",
    ctaTitle: "ابدأ حديثًا معنا",
    ctaLead: "أخبرنا بما تورّده وأين يجب أن يصل.",
    imageAlt: "مزارع يقف في مزرعة بن على سفح تل تحت جبال تغطيها السحب.",
    metaTitle: "عن هيلز",
    metaDescription: "هيلز كوفي شركة لتوريد البن الأخضر وتداوله، وُلدت في دبي وتخدم محامص البن والمستوردين والموزعين في أنحاء المنطقة العربية.",
  },

  contact: {
    eyebrow: "تواصل معنا",
    title: "أخبرنا بما تحتاجه",
    lead: "شاركنا نوع البن والكمية والتوقيت ونقطة التسليم التي تعمل عليها، وسيتابع فريقنا التجاري الحديث معك مباشرة.",
    intentsHeading: "ما الذي يشمله الحديث",
    intents: {
      sourcing: {
        title: "طلب توريد",
        body: "المواصفات والكمية ونقطة التسليم التي تحتاجها. نتوجه إلى مناطق الإنتاج ونعود إليك بما يمكننا توريده.",
      },
      coffee: {
        title: "بن منشور بعينه",
        body: "أي بن في الدليل. نشارك وثائق جودته وشهاداته مع المشترين المعتمدين أثناء الحديث.",
        action: "تصفح الدليل",
      },
      membership: {
        title: "عضوية التداول",
        body: "كيف تعمل العضوية المُراجَعة، وما الغرض من بوابة هيلز للتداول.",
        action: "عن بوابة التداول",
      },
    },
    location: {
      eyebrow: "هيلز في دبي",
      heading: "زُرنا أو اتصل بنا في دبي",
      lead: "يجمع موقعنا في دبي المكتب والمستودع والمصنع في مكان واحد.",
      types: { office: "مكتب", warehouse: "مستودع", factory: "مصنع" },
      city: "دبي، الإمارات العربية المتحدة",
      addressLabel: "العنوان",
      facilitiesLabel: "المرافق",
      hoursLabel: "ساعات العمل",
      hoursValue: "10:00 ص – 7:00 م",
      phoneLabel: "الهاتف",
      primaryPhone: "الخط الرئيسي",
      secondaryPhone: "خط المكتب",
      call: "اتصال",
      channelsHeading: "طرق أخرى للتواصل معنا",
      channels: { email: "البريد الإلكتروني", whatsapp: "واتساب", instagram: "إنستغرام", facebook: "فيسبوك", linkedin: "لينكدإن" },
      openInMaps: "افتح في خرائط Google",
      newTab: "(يُفتح في علامة تبويب جديدة)",
      mapTitle: "خريطة تُظهر موقع هيلز كوفي في داماك سمارت هايتس، دبي",
      mapOpenLabel: "افتح موقع هيلز كوفي في خرائط Google (يُفتح في علامة تبويب جديدة)",
      quickHeading: "تفضّل الاتصال؟",
      quickBody: "يرد فريقنا في دبي خلال ساعات العمل.",
      seeLocation: "اعرض الموقع",
    },
    rfq: {
      heading: "أرسل طلبًا",
      lead: "أخبرنا عن نشاطك التجاري وما الذي تبحث عن توريده. يراجع فريقنا التجاري كل طلب يصل.",
      fields: {
        companyName: "اسم الشركة",
        buyerType: "ما الذي يصف نشاطك التجاري بدقة",
        buyerTypeOptions: {
          roaster: "محمّص بن",
          importer: "مستورد",
          distributor: "موزّع",
          other: "أخرى",
        },
        selectPlaceholder: "اختر خيارًا",
        countryCode: "الدولة",
        countryPlaceholder: "اختر دولتك",
        estimatedVolumeKg: "الكمية التقديرية (كجم)",
        coffeePreference: "تفضيل البن أو المنشأ",
        timing: "التوقيت",
        deliveryLocation: "نقطة التسليم",
        incoterm: "شرط التسليم التجاري (Incoterm)",
        contactName: "اسمك",
        contactEmail: "البريد الإلكتروني",
        contactPhone: "الهاتف",
        message: "أي تفاصيل إضافية؟",
        consent: "أوافق على التواصل معي بخصوص هذا الطلب.",
        optional: "اختياري",
      },
      submit: "إرسال الطلب",
      submitting: "جارٍ الإرسال…",
      unavailable: {
        title: "الإرسال الإلكتروني غير متاح بعد",
        body: "لم نتمكن من إرسال هذا تلقائيًا. لم يُفقد ما أدخلته — يرجى التواصل معنا عبر البيانات أدناه وسنتابع الحديث معك مباشرة.",
      },
      genericError: "حدث خطأ ما. يرجى المحاولة مرة أخرى بعد قليل.",
    },
    detailsHeading: "أين نعمل",
    detailsBody: "هيلز كوفي شركة وُلدت في دبي ولها مكتب تشغيلي في مصر. تُنشر بيانات التواصل الخاصة بهذه الواجهة بعد اعتمادها للنشر.",
    imageAlt: "أكياس بن أخضر مرصوصة على منصات خشبية.",
    metaTitle: "تواصل معنا",
    metaDescription: "ابدأ حديث توريد مع هيلز كوفي: شاركنا نوع البن والكمية والتوقيت ونقطة التسليم التي تعمل عليها.",
  },

  coffee: {
    index: {
      eyebrow: "الدليل",
      title: "البن",
      statCoffees: "القهوة المنشورة",
      statOrigins: "المناشئ الممثَّلة",
      statProcesses: "طرق المعالجة",
      lead: "كل بن تنشره هيلز حاليًا، مع منشئه وطريقة معالجته وتعبئته. هذا دليل للتوريد، وليس دفتر طلبات مباشرًا.",
      empty: "لا يوجد بن منشور في الوقت الحالي. أرسل لنا طلب توريد وسنخبرك بما يمكننا توريده.",
      metaTitle: "البن",
      metaDescription: "أنواع البن الأخضر التي تنشرها هيلز كوفي، مع المنشأ وطريقة المعالجة والتعبئة والشهادات للمشترين المحترفين.",
      filter: {
        searchLabel: "ابحث في البن المنشور",
        searchPlaceholder: "البن أو المنشأ",
        originFacet: "المنشأ",
        processFacet: "طريقة المعالجة",
        typeFacet: "نوع البن",
        all: "الكل",
        clear: "مسح التصفية",
        resultCount: "عرض {shown} من أصل {total} من البن المنشور",
        noResults: "لا يوجد بن منشور يطابق هذه التصفية. امسحها لعرض الدليل كاملًا.",
      },
    },
    detail: {
      specHeading: "المواصفات",
      coffeeType: "نوع البن",
      variety: "الصنف",
      processingMethod: "طريقة المعالجة",
      packaging: "التعبئة",
      region: "المنطقة",
      country: "الدولة",
      tagsHeading: "الخصائص",
      certificationsHeading: "الشهادات",
      certificationValidUntil: "سارية حتى",
      traceabilityHeading: "التتبع",
      traceabilityBody:
        "تحافظ هيلز على الصلة بين هذا البن والمنطقة التي زُرع فيها. تُشارَك وثائق الجودة والشهادات مع المشترين المعتمدين أثناء الحديث التجاري، ولا ننشر إلا ما يمكننا إثباته.",
      rfqHeading: "مهتم بهذا البن؟",
      rfqLead: "أرسل لنا الكمية والتوقيت ونقطة التسليم التي تحتاجها وسنعود إليك بما يمكننا توريده.",
      notSpecified: "غير محدد",
      backToIndex: "كل البن",
      identityEyebrow: "بن منشور",
      originLinkAction: "اطّلع على المنشأ",
      originConnectionHeading: "من أين يأتي",
      specIdentityHeading: "القهوة",
      specProcessingHeading: "المعالجة والتعبئة",
      specOriginHeading: "المنشأ",
      originLabel: "المنشأ",
      specLead: "ما تستطيع هيلز إثباته اليوم عن هذه القهوة. تُشارَك مستندات الجودة مع المشترين المعتمدين خلال المحادثة التجارية.",
      commercialEyebrow: "السياق التجاري",
      commercialTitle: "كيف تُشترى هذه القهوة",
      commercialLead:
        "لا تنشر هيلز سعر قائمة لقهوة الكتالوج. تُسعَّر الإمدادات عبر محادثة تجارية، ويتداول الأعضاء المعتمدون عروض البائعين المباشرة داخل بوابة التداول.",
      referenceHeading: "بيانات السوق المرجعية",
      referenceNote: "المؤشرات المرجعية معلومات سوقية للسياق فقط. ليست عرضًا ولا سعر بيع لهذه القهوة.",
      offersHeading: "عروض البائعين المباشرة",
      offersBody:
        "عروض البائعين لهذه القهوة — السعر لكل كيلوغرام والكمية المتاحة والمستودع الذي يحفظها — تظهر فقط للأعضاء المعتمدين داخل بوابة تداول هيلز، ولا تُنشر أبدًا على الموقع العام.",
      offersSignIn: "تسجيل الدخول إلى بوابة التداول",
      offersApply: "التقدّم بطلب عضوية",
      rfqCardHeading: "اطلب عرضًا من هيلز",
      rfqCardBody: "أخبرنا بالكمية والتوقيت ونقطة التسليم، وسيرد الفريق التجاري بما تستطيع هيلز توفيره من هذه القهوة.",
      heroCommercialLink: "الخيارات التجارية",
      imageAlt: "{name} — قهوة خضراء",
    },
  },

  origins: {
    index: {
      eyebrow: "المناشئ",
      title: "المناشئ",
      statOrigins: "المناشئ النشطة",
      statCoffees: "القهوة المنشورة",
      lead: "مناطق الإنتاج التي تورّد منها هيلز. يرتبط كل ملف بالبن الذي ننشره حاليًا من ذلك المنشأ.",
      empty: "لا توجد مناشئ منشورة في الوقت الحالي.",
      metaTitle: "المناشئ",
      metaDescription: "مناطق الإنتاج التي تورّد منها هيلز كوفي البن الأخضر، والبن المنشور من كل منشأ.",
    },
    detail: {
      regionLabel: "المنطقة",
      partOfLabel: "جزء من",
      coffeesHeading: "البن من هذا المنشأ",
      coffeesEmpty: "لا يوجد بن منشور من هذا المنشأ في الوقت الحالي. أرسل لنا طلب توريد وسنخبرك بما يمكننا توريده.",
      backToIndex: "كل المناشئ",
      identityEyebrow: "منطقة إنتاج",
      countryLabel: "الدولة",
      exploreCoffee: "تصفح كل البن",
      dossierHeading: "ملف المنشأ",
      coffeesCountLabel: "القهوة المنشورة",
      coffeesCountValue: "{count}",
      mediaCaption: "قهوة من هذا المنشأ",
      coffeesLead: "القهوة التي تنشرها هيلز حاليًا من هذا المنشأ. يفتح كل منها ملفه الكامل بالمواصفات والخيارات التجارية.",
      emptyTitle: "لا توجد قهوة منشورة من هذا المنشأ حاليًا",
      sourcingEyebrow: "التوريد من هذا المنشأ",
      sourcingTitle: "ما تتحمّل هيلز مسؤوليته",
      closingTitle: "تبحث عن توريد من هذا المنشأ؟",
      closingBody: "أخبرنا بالكمية والتوقيت ونقطة التسليم، وسيرد الفريق التجاري بما تستطيع هيلز توفيره من هنا.",
    },
    showcase: {
      previous: "المناشئ السابقة",
      next: "المناشئ التالية",
      trackLabel: "المناشئ",
      progressLabel: "موضع العرض",
    },
  },

  sourcing: {
    eyebrow: "التوريد",
    title: "كيف نورّد البن ونحتفظ به",
    lead: "توريد البن الأخضر سلسلة من القرارات تُتخذ قبل أشهر من وصول الكيس إلى المحمصة. هكذا تتخذها هيلز.",
    relationships: {
      title: "علاقات المنشأ",
      body: "نعمل مباشرة مع مناطق الإنتاج التي نورّد منها، فيحمل البن الذي ننشره صلة حقيقية قابلة للتتبع بمكان زراعته.",
    },
    custody: {
      title: "الحيازة",
      body:
        "ينتقل البن الذي تورّده هيلز إلى حيازة معتمدة من هيلز ويبقى فيها حتى تنتقل الملكية إلى المشتري. لا تُقسَّم المسؤولية عن البن ولا تبقى غامضة.",
    },
    logistics: {
      title: "الخدمات اللوجستية",
      body: "يتم التسليم عبر ترتيبات لوجستية اعتمدتها هيلز، بما يبقي الإمداد محسوبًا بين المنشأ ونقطة التسليم التي يختارها المشتري.",
    },
    quality: {
      title: "توثيق الجودة",
      body: "تُجمع الشهادات وأدلة الجودة وتُراجَع قبل نشر أي بن، وتُشارَك مع المشترين المعتمدين كجزء من الحديث التجاري.",
    },
    cta: {
      title: "تحدث معنا عن بن بعينه",
      lead: "إذا أردت التعمق في أي من ذلك بشأن بن محدد، فيسعدنا أن نجري هذا الحديث.",
    },
    images: {
      harvestAlt: "ثمار بن ناضجة على الغصن في المنشأ.",
      qualityAlt: "بن أخضر يُفحص يدويًا للتحقق من الجودة.",
    },
    metaTitle: "التوريد",
    metaDescription: "كيف تورّد هيلز كوفي البن الأخضر من المنشأ وتحتفظ به في حيازة معتمدة للمشترين المحترفين في أنحاء المنطقة العربية.",
  },

  portalEntry: {
    eyebrow: "بوابة التداول",
    title: "انضم إلى بوابة تداول هيلز",
    lead: "بوابة التداول هي المكان الذي يشتري فيه الأعضاء المعتمدون البن الأخضر ويحتفظون به ويعيدون بيعه تحت عهدة معتمدة من هيلز.",
    stepsHeading: "كيف تعمل العضوية",
    steps: {
      account: { title: "أنشئ حسابًا", body: "سجّل وأكّد عنوان بريدك الإلكتروني." },
      organization: { title: "أعدّ منظمتك", body: "أضف شركتك وقدّم مستندات التحقق الخاصة بها." },
      review: { title: "مراجعة من هيلز", body: "تُراجَع كل منظمة وتُعتمد يدويًا. التسجيل وحده لا يتيح التداول أبدًا." },
      trade: { title: "تداول في السوق", body: "بعد الاعتماد، شاهد الإدراجات المباشرة واشترِ أو احتفظ أو أعد البيع." },
    },
    createAccount: "أنشئ حسابًا",
    signIn: "تسجيل الدخول",
    memberPrompt: "عضو بالفعل؟",
    contactPrompt: "تفضّل الحديث أولًا؟",
    contactAction: "تواصل مع الفريق التجاري",
    imageAlt: "أكياس بن أخضر محفوظة في مستودع دافئ.",
    metaTitle: "بوابة التداول",
    metaDescription: "كيف تنضم إلى بوابة تداول هيلز: أنشئ حسابًا، وأعدّ منظمتك، وأكمل مراجعة هيلز.",
  },

  notFound: {
    eyebrow: "الصفحة غير موجودة",
    title: "هذه الصفحة غير متاحة.",
    body:
      "قد يكون العنوان غير صحيح، أو قد لا تكون الصفحة منشورة بعد الآن. يمكنك العودة إلى الصفحة الرئيسية أو تصفح دليل البن المنشور.",
    homeAction: "العودة إلى الرئيسية",
    coffeeAction: "تصفح البن",
  },

  referencePrice: {
    label: "السعر المرجعي",
    unavailableTitle: "لم يُنشر السعر المرجعي بعد",
    unavailableBody: "سيعرض هذا القسم سعرًا مرجعيًا قياسيًا حين تنشر هيلز واحدًا لهذا البن.",
    stageLabel: "معلومات مرجعية",
    stageState: "لم تُنشر",
    stageNote: "المعلومات المرجعية غير متاحة للعرض بعد.",
    disclosure: "معلومات مرجعية، وليست عرضًا.",
  },

  pricing: {
    listLabel: "الأسعار المرجعية القياسية",
    referenceOnly: "معلومات مرجعية، وليست عرضًا.",
    fields: {
      source: "المصدر",
      value: "القيمة المُبلَّغ عنها",
      unit: "الوحدة",
      currency: "العملة",
      observed: "وقت الرصد",
      timeZone: "المنطقة الزمنية",
      delay: "توقيت البيانات",
    },
    delayTypes: {
      REAL_TIME: "لحظي",
      DELAYED: "متأخر",
      DAILY: "يومي",
      MANUAL: "مُدخَل يدويًا",
    },
    delayMinutes: "دقيقة تأخير",
    commodities: {
      ARABICA: "أرابيكا",
      ROBUSTA: "روبوستا",
      ICO_INDICATOR: "مؤشر المنظمة الدولية للبن",
    },
    rawValuesNote:
      "تُعرض القيم كما سجّلها المصدر تمامًا، بوحدته وعملته الأصليتين. لا يُطبَّق أي تحويل للعملة أو للوحدة.",
    stale: {
      title: "بيانات مرجعية قديمة",
      body: "هذا المصدر موسوم بأنه قديم، لذلك لا تُعرض آخر قيمة له على أنها حالية.",
      lastSuccess: "آخر رصد ناجح",
    },
    unavailable: {
      label: "السعر المرجعي",
      noSource: {
        title: "لم يُنشر السعر المرجعي بعد",
        body: "لا يوجد مصدر مرجعي معتمد. سيظهر هنا سعر مرجعي قياسي حين تنشر هيلز واحدًا.",
      },
      noObservation: {
        title: "لا يوجد رصد مرجعي متاح",
        body: "يوجد مصدر معتمد لكنه لم ينشر رصدًا للعرض. لا يُقدَّر شيء بدلًا منه.",
      },
      incomplete: {
        title: "حُجب السعر المرجعي",
        body: "لا تُعرض القيمة إلا مع إفصاحها الكامل، وهذا الإفصاح غير مكتمل.",
      },
      readFailed: {
        title: "السعر المرجعي غير متاح مؤقتًا",
        body: "تعذّر تحميل السعر المرجعي الآن. لا تُعرض أي قيمة ولا يُقدَّر شيء.",
      },
      lastSuccess: "آخر رصد ناجح",
    },
    basis: {
      title: "أساس السعر (للتوضيح فقط)",
      lead: "كيف ترتبط القيمة المرجعية بالفروقات المسجّلة. هذا توضيح لأساس السعر — وليس عرض سعر ولا عرضًا تجاريًا.",
      benchmarkHeading: "القيمة المرجعية",
      differentialsHeading: "الفروقات المسجّلة",
      notSummed:
        "تُعرض المبالغ كما سُجّلت، كلٌّ بوحدته وعملته. لا تُجمع ولا تُقاصّ ولا تُحوَّل.",
      effective: "سارٍ",
      from: "من",
      until: "حتى",
      openEnded: "بلا تاريخ انتهاء",
      types: {
        ORIGIN: "المنشأ",
        QUALITY: "الجودة",
        CERTIFICATION: "الاعتماد",
        CROP: "المحصول",
        COMMERCIAL: "تجاري",
        OTHER: "أخرى",
      },
    },
  },

  auth: {
    signIn: {
      eyebrow: "بوابة التداول",
      title: "تسجيل الدخول",
      lead: "سجّل الدخول بحساب هيلز الخاص بك للمتابعة إلى مساحة عمل مؤسستك.",
      email: "البريد الإلكتروني",
      password: "كلمة المرور",
      submit: "تسجيل الدخول",
      submitting: "جارٍ تسجيل الدخول…",
      forgotPassword: "نسيت كلمة المرور؟",
      genericError: "تركيبة البريد الإلكتروني وكلمة المرور هذه غير معروفة. يرجى المحاولة مرة أخرى.",
      serverError: "تعذر تسجيل دخولك. يرجى المحاولة مرة أخرى.",
      adminPortalRequired: "هذا الحساب مخصص لبوابة الإدارة. يُرجى تسجيل الدخول من بوابة الإدارة.",
      adminPortalAction: "فتح بوابة الإدارة",
      metaTitle: "تسجيل الدخول",
      noAccount: "ليس لديك حساب؟",
      createAccount: "إنشاء حساب",
    },
    adminSignIn: {
      eyebrow: "بوابة الإدارة",
      title: "تسجيل دخول التشغيل",
      lead: "سجّل الدخول بحساب مسؤول Hills Coffee الخاص بك.",
      email: "البريد الإلكتروني",
      password: "كلمة المرور",
      submit: "تسجيل الدخول",
      submitting: "جارٍ تسجيل الدخول…",
      forgotPassword: "نسيت كلمة المرور؟",
      genericError: "تركيبة البريد الإلكتروني وكلمة المرور هذه غير معروفة. يرجى المحاولة مرة أخرى.",
      accessDenied: "لا يمكن لهذا الحساب الوصول إلى بوابة الإدارة.",
      serverError: "تعذر تسجيل دخولك. يرجى المحاولة مرة أخرى.",
      metaTitle: "تسجيل دخول المسؤول",
    },
    signUp: {
      eyebrow: "أنشئ حسابك",
      title: "إنشاء حساب",
      lead: "أنشئ حساب هيلز الخاص بك لبدء طلب العضوية.",
      fullName: "الاسم الكامل",
      email: "البريد الإلكتروني",
      password: "كلمة المرور",
      confirmPassword: "تأكيد كلمة المرور",
      submit: "إنشاء حساب",
      submitting: "جارٍ إنشاء الحساب…",
      acknowledgement: "تحقق من بريدك الإلكتروني لتأكيد حسابك والمتابعة.",
      invalidEmail: "أدخل عنوان بريد إلكتروني صالحًا.",
      weakPassword: "اختر كلمة مرور أقوى ثم حاول مرة أخرى.",
      rateLimited: "هناك محاولات كثيرة. انتظر قليلًا ثم حاول مرة أخرى.",
      genericError: "تعذر إنشاء حسابك. يرجى المحاولة مرة أخرى.",
      haveAccount: "لديك حساب بالفعل؟",
      afterSignUpNote: "بعد إنشاء حسابك، سنتحقق من بريدك الإلكتروني قبل إعداد النشاط التجاري.",
      metaTitle: "إنشاء حساب",
    },
    password: {
      show: "إظهار كلمة المرور",
      hide: "إخفاء كلمة المرور",
    },
    signOutConfirm: {
      title: "تسجيل الخروج؟",
      description: "ستحتاج إلى تسجيل الدخول مرة أخرى للوصول إلى مساحة عملك.",
      cancel: "إلغاء",
      confirm: "تسجيل الخروج",
      confirming: "جارٍ تسجيل الخروج…",
    },
    verifyEmail: {
      eyebrow: "خطوة أخيرة",
      title: "تحقق من بريدك الإلكتروني",
      lead: "أرسلنا رابط تحقق إلى بريدك الإلكتروني. افتحه لتأكيد حسابك قبل المتابعة.",
      resend: "إعادة إرسال رابط التحقق",
      resending: "جارٍ الإرسال…",
      resent: "تم إرسال رسالة التحقق — تحقق من بريدك الوارد.",
      resendFailed: "تعذر إرسال رسالة التحقق. يرجى المحاولة بعد قليل.",
      sessionExpired: "يرجى تسجيل الدخول مرة أخرى لطلب رسالة تحقق جديدة.",
      metaTitle: "تحقق من بريدك الإلكتروني",
      gatedTitle: "تحقق من بريدك الإلكتروني للمتابعة",
      gatedDescription: "تتطلب هذه المنطقة عنوان بريد إلكتروني مؤكدًا. تحقق من صندوق الوارد لرابط التحقق، أو اطلب رابطًا جديدًا.",
    },
    resetPassword: {
      eyebrow: "استعادة الحساب",
      title: "إعادة تعيين كلمة المرور",
      lead: "أدخل بريد حسابك الإلكتروني وسنرسل لك رابطًا لإعادة تعيين كلمة المرور.",
      email: "البريد الإلكتروني",
      submit: "إرسال رابط إعادة التعيين",
      submitting: "جارٍ الإرسال…",
      acknowledgement: "إذا كان هناك حساب مرتبط بهذا البريد الإلكتروني، فقد أُرسل إليه رابط إعادة تعيين كلمة المرور.",
      metaTitle: "إعادة تعيين كلمة المرور",
      confirmTitle: "اختر كلمة مرور جديدة",
      confirmLead: "أدخل كلمة مرور جديدة لحسابك.",
      newPassword: "كلمة المرور الجديدة",
      confirmNewPassword: "تأكيد كلمة المرور الجديدة",
      confirmSubmit: "حفظ كلمة المرور الجديدة",
      confirmSubmitting: "جارٍ الحفظ…",
      confirmSuccess: "تم تحديث كلمة مرورك. يمكنك تسجيل الدخول بها الآن.",
      passwordMismatch: "كلمتا المرور غير متطابقتين.",
      invalidLink: "رابط إعادة التعيين هذا غير صالح أو منتهي الصلاحية. اطلب رابطًا جديدًا.",
      genericError: "تعذر إكمال الطلب. يرجى المحاولة مرة أخرى.",
      metaTitleConfirm: "اختر كلمة مرور جديدة",
    },
    mfa: {
      challengeEyebrow: "تحقق إضافي",
      challengeTitle: "أدخل رمز المصادقة",
      challengeLead: "افتح تطبيق المصادقة وأدخل الرمز المكوّن من 6 أرقام لإتمام تسجيل الدخول.",
      code: "رمز المصادقة",
      verify: "تحقق",
      verifying: "جارٍ التحقق…",
      invalidCode: "هذا الرمز غير صحيح. يرجى المحاولة مرة أخرى.",
      metaTitleChallenge: "تحقق من هويتك",
      enrollEyebrow: "أمان الحساب",
      enrollTitle: "إعداد المصادقة الثنائية",
      enrollLead: "امسح هذا الرمز بتطبيق المصادقة، ثم أدخل الرمز المكوّن من 6 أرقام الذي يولّده لإتمام الإعداد.",
      enrollConfirm: "تأكيد وتفعيل",
      enrollConfirming: "جارٍ التأكيد…",
      enrollSuccess: "تم تفعيل المصادقة الثنائية على حسابك الآن.",
      alreadyEnrolled: "المصادقة الثنائية مفعّلة بالفعل على حسابك.",
      manageInAccount: "العودة إلى أمان الحساب",
      manualKeyLabel: "لا يمكنك المسح؟ أدخل هذا المفتاح في تطبيق المصادقة:",
      metaTitleEnroll: "إعداد المصادقة الثنائية",
    },
    layout: {
      backToSite: "العودة إلى هيلز كوفي",
    },
  },

  account: {
    signIn: "تسجيل الدخول",
    signUp: "إنشاء حساب",
    menuLabel: "قائمة الحساب",
    dashboard: "لوحة التحكم",
    adminConsole: "وحدة التشغيل",
    signOut: "تسجيل الخروج",
    chooseOrganization: "اختر المؤسسة",
  },
};
